...
for n_type in range(1):
    ...
    dir_singlewheel = "D:/HuaweiMoveData/Users/17908/Desktop/Data/RoveR_Data/slip="
    #dir2 = dir2_hmmwv
    #if wheel_type == "VIPER_Wheel":
        #dir2 = dir2_viper
    ...
    for k in range(3):
        ...
        y_scm = DBP_Torque_Mean_SCM[1][:] / 6.0
        if k == 1:
            y_crm = DBP_Torque_Mean[2][:]
            y_scm = DBP_Torque_Mean_SCM[2][:] / 6.0
        if k == 2: 
            y_crm = 180 / math.pi * np.arctan(1.0 / tot_load * DBP_Torque_Mean[1][:])
            y_scm = 180 / math.pi * np.arctan(1.0 / tot_load * DBP_Torque_Mean_SCM[1][:] / 6.0) 
        plt.plot(x_crm, y_crm, 'rs--', lw = lw, markersize = ms, label = "DEM")
